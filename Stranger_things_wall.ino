...
#include "lang_en.h"; // change to your language
...
#define LETTER_DURATION 1000 // => 1 second
#define PAUSE_DURATION 15000 // => 15 seconds by default
#define MAX_PAUSE_DURATION 60000 // => 1 minute
...
String getPage(){
  ...
  page += "<p>" + String(TEXT_LETTER_DURATION) + " (ms) : " + String(letterDuration) + "</p>";
  page += "<input type='range' min='500' max='" + String(LETTER_DURATION*2) + "' value='" + String(letterDuration) + "' name='letterduration' class='slider'>";
  
  page += "<p>" + String(TEXT_PAUSE_DURATION)  + " (ms) : " + String(pauseDuration) + "</p>";
  page += "<input type='range' min='0' max='" + String(MAX_PAUSE_DURATION) + "' value='" + String(pauseDuration) + "' name='pauseduration' class='slider'><br><br>";
  ...
  page += "<input type='range' min='0' max='255' value='" + String(brightness) + "' name='brightness' class='slider'><br><br>";
...
void loop(void){
  ...
  // each letter duration
  if(millis() - timer >= letterDuration) {
    if(messageLetterIndex >= message.length()) {
      // if all letters displayed, go into normal mode
      if(millis() - normalTimer >= pauseDuration) {
        // after normal moment, diplays colors for PAUSE_DURATION ms

         // made so that the first letter is directly displayed
        timer = millis() + letterDuration;

        Serial.println("reset normal");
        messageLetterIndex = 0;
      } else {
        // during normal moment, trigger normal leds
                
        if(!doNotShow)
          doNotShowLeds();
      }
    } else {
      // not all letters have been displayed, continue please
